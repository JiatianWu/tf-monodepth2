...
class KinectDataSource:
    ...
    def run_rgb_only(self):
        ...
        while not self.reader.is_eof():
            ...
            if self.output is not None:
                ...
                color_filename = '{0}/{1:06d}.jpg'.format(
                    self.output, idx)
    ...
    def run(self):
        if self.output is not None:
            ...
            with open('intrinsic.json') as json_file:
        ...
        while not self.reader.is_eof():
            ...
            if self.output is not None:
                ...
                if len(image_seq) < 2:
                    ...
                    intrinsics_filename = '{0}/{1:06d}_cam.txt'.format(self.output, idx)
...
if __name__ == '__main__':
    ...
    if args.output is None:
        ...
        try:
            ...
            dirlist = sorted(os.listdir(args.input))
            for seq in dirlist:
                if '.mkv' in seq:
                    os.mkdir('{}/{}'.format(args.output, seq[:-4]))
                    reader = KinectDataSource(args.input + '/' + seq, args.output + '/' + seq[:-4])
                    reader.run()
