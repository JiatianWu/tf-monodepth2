...
class lyft_loader(object):
    ...
    def collect_train_frames(self):
        ...
        for date in self.date_list:
            ...
            for frame in frame_list:
                if '.png' in frame:
                    frame_id = frame
                    all_frames.append(date + ' ' + frame_id[:-4])
    ...
    def load_image_raw(self, drive, frame_id):
        img_file = self.dataset_dir + drive + '/' + frame_id + '.png'
        img = Image.open(img_file).crop((0, 0, 1224, 700))
        return img

    def load_intrinsics_raw(self, drive, frame_id):
        data_file = self.dataset_dir + drive + '/' + frame_id + '.npy'
        P_rect = np.load(data_file)
        return P_rect
