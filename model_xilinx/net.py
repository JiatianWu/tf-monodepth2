...
class Net(object):
    ...
    def build_disp_net(self, res18_tc, skips):
        ...
        with tf.variable_scope('depth_decoder', reuse=tf.AUTO_REUSE) as scope:
            ...
            with slim.arg_scope([slim.conv2d],
                ...
                disp4 = self._conv_reflect(upconv4, 3, 1, 1, 'disp4', activation_fn=tf.nn.sigmoid, type='sigmoid')
                ...
                disp3 = self._conv_reflect(upconv3,3, 1, 1, 'disp3', activation_fn=tf.nn.sigmoid, type='sigmoid')
                ...
                disp2 = self._conv_reflect(upconv2,3, 1, 1, 'disp2', activation_fn=tf.nn.sigmoid, type='sigmoid')
                ...
                disp1 = self._conv_reflect(upconv1, 3, 1, 1, 'disp1', activation_fn=tf.nn.sigmoid, type='sigmoid')

                return [disp1, disp2, disp3, disp4]

    def build_disp_net_nosigmoid(self, res18_tc, skips):
        print('Building Depth Decoder Model')
        with tf.variable_scope('depth_decoder', reuse=tf.AUTO_REUSE) as scope:
            end_points_collection = scope.original_name_scope + '_end_points'
            with slim.arg_scope([slim.conv2d],
                ...
                iconv1_concat = iconv1_upsample
                upconv1 = self._conv_reflect(iconv1_concat,3, filters[0],1,'upconv1')
                disp1 = self._conv_reflect(upconv1, 3, 1, 1, 'disp1', activation_fn=tf.nn.sigmoid, type='none')

                return [disp1, disp2, disp3, disp4]

    def build_disp_net_nosigmoid_bilinear(self, res18_tc, skips):
        print('Building Depth Decoder Model')
        with tf.variable_scope('depth_decoder', reuse=tf.AUTO_REUSE) as scope:
            end_points_collection = scope.original_name_scope + '_end_points'
            with slim.arg_scope([slim.conv2d],
                ...
                # disp 5
                iconv5 = self._conv_reflect(res18_tc, 3, filters[4], 1, 'iconv5')
                iconv5_upsample = tf.image.resize(iconv5, [np.int(self.H / 16), np.int(self.W / 16)])
                iconv5_concat = tf.concat([iconv5_upsample, skips[0]], axis=3)
                upconv5 = self._conv_reflect(iconv5_concat, 3,  filters[4], 1, 'upconv5')

                # disp 4
                iconv4 = self._conv_reflect(upconv5, 3, filters[3], 1, 'iconv4')
                iconv4_upsample = tf.image.resize(iconv4, [np.int(self.H / 8), np.int(self.W / 8)])
                iconv4_concat = tf.concat([iconv4_upsample, skips[1]], axis=3)
                upconv4 = self._conv_reflect(iconv4_concat,3, filters[3], 1, 'upconv4')
                disp4 = self._conv_reflect(upconv4, 3, 1, 1, 'disp4', activation_fn=tf.nn.sigmoid, type='sigmoid')
                ...
                # disp 3
                iconv3 = self._conv_reflect(upconv4,3, filters[2], 1, 'iconv3')
                iconv3_upsample = tf.image.resize(iconv3, [np.int(self.H / 4), np.int(self.W / 4)])
                iconv3_concat = tf.concat([iconv3_upsample, skips[2]], axis=3)
                upconv3 = self._conv_reflect(iconv3_concat,3, filters[2],1, 'upconv3')
                disp3 = self._conv_reflect(upconv3,3, 1, 1, 'disp3', activation_fn=tf.nn.sigmoid, type='sigmoid')
                ...
                # disp 2
                iconv2 = self._conv_reflect(upconv3,3, filters[1],1, 'iconv2')
                iconv2_upsample = tf.image.resize(iconv2, [np.int(self.H / 2), np.int(self.W / 2)])
                iconv2_concat = tf.concat([iconv2_upsample, skips[3]], axis=3)
                upconv2 = self._conv_reflect(iconv2_concat,3, filters[1], 1, 'upconv2')
                disp2 = self._conv_reflect(upconv2,3, 1, 1, 'disp2', activation_fn=tf.nn.sigmoid, type='sigmoid')
                ...
                # disp 1
                iconv1 = self._conv_reflect(upconv2,3, filters[0], 1, 'iconv1')
                iconv1_upsample = tf.image.resize(iconv1, [np.int(self.H), np.int(self.W)])
                iconv1_concat = iconv1_upsample
                upconv1 = self._conv_reflect(iconv1_concat,3, filters[0],1,'upconv1')
                disp1 = self._conv_reflect(upconv1, 3, 1, 1, 'disp1', activation_fn=tf.nn.sigmoid, type='none')
    ...
    def build_disp_net_bilinear(self, res18_tc, skips):
        print('Building Depth Decoder Model')
        with tf.variable_scope('depth_decoder', reuse=tf.AUTO_REUSE) as scope:
            end_points_collection = scope.original_name_scope + '_end_points'
            with slim.arg_scope([slim.conv2d],
    ...
    def _conv_reflect(self, x, filter_size, out_channel, stride, name='conv', activation_fn=tf.nn.relu, type='relu'):
        ...
        x = slim.conv2d(pad_x, out_channel, [filter_size, filter_size], stride, padding='VALID', scope=name)
        with tf.variable_scope(name, reuse=tf.AUTO_REUSE) as scope:
            x = tfl.batch_normalization(x, momentum=self.decay, epsilon=self.epsilon,training=self.is_training, name='BatchNorm', fused=True,reuse=tf.AUTO_REUSE)
            if type == 'relu':
